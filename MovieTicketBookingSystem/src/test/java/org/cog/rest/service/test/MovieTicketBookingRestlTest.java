...
public class MovieTicketBookingRestlTest {
    ...
    @Test
    public void testCheckTicketAvability() throws IOException {
        Client client = Client.create(new DefaultClientConfig());
        WebResource service = client.resource(getBaseURI());
        String resp = service.path("MovieTicketBookingSystem").path("rest").path("movieTicketServices").path("checkAval").path(ShowTime._11AM.toString())
    ...
    public void bookTicket() throws IOException {
        Client client = Client.create(new DefaultClientConfig());
        WebResource service = client.resource(getBaseURI()).path("MovieTicketBookingSystem").path("rest").path("movieTicketServices").path("bookTicket");
